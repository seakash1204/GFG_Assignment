const clothingStock = [
   ...
   clothingStock.map(({color, category})=>{
    category === 'women shirt' && console.log(color)
   })

   // 3b. Console all the different colours available for men shirt which are currently in stock.
   clothingStock.map(({inStock, color, category})=>{
        category === 'men shirt' && inStock === true && console.log(color);
   })

   // 3c: Console an array which only contains the elements which are in stock.
   clothingStock.filter((product)=>{
    product.inStock && console.log(product)
   })